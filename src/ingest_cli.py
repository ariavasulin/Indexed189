...
class NotebookProcessor:
    ...
    def _save_image(self, data: str, fig_dir: Path, name: str) -> Path:
        fig_dir.mkdir(parents=True, exist_ok=True)
        image_path = fig_dir / name
        ...
        for output in outputs:
            ...
            if output_type in {"display_data", "execute_result"}:
                ...
                if "image/png" in data:
                    img_name = f"cell-{cell_index:03d}-output.png"
                    img_path = self._save_image(data["image/png"], fig_dir, img_name)
                    rel_path = os.path.relpath(img_path, self.layout.searchable)
                    buffer.append(f"![Output]({rel_path})\n\n")
                elif "image/jpeg" in data:
                    img_name = f"cell-{cell_index:03d}-output.jpg"
                    img_path = self._save_image(data["image/jpeg"], fig_dir, img_name)
                    rel_path = os.path.relpath(img_path, self.layout.searchable)
                    buffer.append(f"![Output]({rel_path})\n\n")
    ...
    def process(self, notebook_path: Path) -> Path:
        ...
        fig_dir = self.layout.figures / slug
        fig_dir.mkdir(parents=True, exist_ok=True)
        ...
        for cell in notebook.cells:
            ...
            if cell_type == "markdown":
                ...
                if outputs:
                    code_cell_index += 1
                    lines.append(
                        self._process_outputs(outputs, fig_dir, slug, code_cell_index)
                    )
...
def process_documents(
    ...
    if annotate and new_markdown:
        annotator = VisionAnnotator()
        for md_file in new_markdown:
            try:
                annotated, skipped = annotator.annotate_markdown(md_file)
                print(
                    f"Annotated {annotated} image(s) and skipped {skipped} in {md_file.name}"
                )
            except Exception as exc:
                print(f"Failed to annotate {md_file.name}: {exc}")

    if not new_markdown:
        print("No new markdown files generated.")
    else:
        print("\nGenerated markdown files:")
        for path in new_markdown:
            print(f" - {path.relative_to(layout.searchable)}")
...
def parse_args(args: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--annotate",
        action="store_true",
        help="Annotate generated markdown images with OpenAI Vision",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    process_documents(layout, args.types, args.annotate)
